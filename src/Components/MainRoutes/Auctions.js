...
const Auctions = () => {
  ...
  let historicAuctions = allAuctions.filter(on => Date.parse(on.SlutDatum) < date.getTime());
  ...
  // && (auction.Titel.toLowerCase().includes(search.toLowerCase()) || !search))
  function getHistoric() {
    return (
        historicAuctions.filter(auction => auction)
              .map(auction => {
              return (
                <div className="col-sm-4" key={auction.AuktionID}>
    ...
    function getOngoingWithQuery() {
      return (
        ongoingAuctions.filter(auction => auction.Titel.toLowerCase().includes(search.toLowerCase()) || !search)
          .map(auction => {
          return(
          <div className="col-sm-4" key={auction.AuktionID}>
          <div className="card activeCard">
  ...
  function getHistoricWithQuery() {
    return (
        historicAuctions.filter(auction => auction.Titel.toLowerCase().includes(search.toLowerCase()) || !search)
          .map(auction => {
            return (
              <div className="col-sm-4" key={auction.AuktionID}>
                <div className="card historicCard">
  ...
  if (id === "all") {
    ...
    return (
        <div className="auctionContainer container">
          <h1 className="text-center">Alla auktioner</h1>
          <div className="row justify-content-center">
            {getOngoingWithQuery()}
            {getHistoricWithQuery()}
          </div>
        </div>
      );
  }
